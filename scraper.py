...
def render(date_labels, locations, trucks):
    ...
    thead = "<tr><th class=\"loc-col\">Location</th>" + "".join(f"<
def fmt_date(d):
    return f"{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][d.weekday()]} {d.month}/{d.day}"
...
def parse_mellow_shop(shop_url):
    ...
    We deliberately don't scrape the free-text bio here -- it's Japanese
    prose with no reliable markers to isolate, and a partial/garbled scrape
    reads worse in the directory than a plain, consistent English line.
    build() supplies a fixed description instead.
    """
    if shop_url in SHOP_CACHE:
        return SHOP_CACHE[shop_url]
    htmltxt = fetch(shop_url)
        ...
        for e in entries:
            for d, label in zip(dates, date_labels):
                if e["next_date"] == d:
                    ...
                    name = info["name"] or e["raw"][:24]
                    dish = dish_from_raw(e["raw"], info["name"])
                    register_truck(name, "Mellow SHOP STOP kitchen car.",
                                    dish, e["shop_url"], info.get("img"))
                    sched[label].append(name)
        note = None
        if not entries:
...
def parse_mellow_shop(shop_url):
    """Fetch a shop's own page for canonical name / photo / short blurb."""
    if shop_url in SHOP_CACHE:
        return SHOP_CACHE[shop_url]
    htmltxt = fetch(shop_url)
    info = {"name": None, "img": None, "blurb": None}
    if htmltxt:
        soup = BeautifulSoup(htmltxt, "html.parser")
        og_title = soup.find("meta", attrs={"property": "og:title"})
        if og_title and og_title.get("content"):
        ...
        if og_img and og_img.get("content"):
            info["img"] = og_img["content"]
        # heuristic: the free-text blurb sits between the SNS/HP line and the
        # "favorite" CTA link. Fall back to None if the markers move.
        text = soup.get_text("\n", strip=True)
        m = re.search(r"\u767b\u9332\u304c\u3042\u308a\u307e\u305b\u3093\n(.+?)\nSHOP\s*STOP",
                       text, re.S)
        if m:
    ...
    for d in dates:
        ...
        for img in soup.find_all("img", src=re.compile("car_photo_thum")):
            ...
            mm = re.search(r"\u3010\u30e1\u30cb\u30e5\u30fc\uff1a(.+?)\u3011(.*?)(?=$|\n\n)",
                            snippet, re.S)
            dish = mm.group(1).split("/")[0].strip() if mm else "Menu on-site"
            desc = re.sub(r"\s+", " ", mm.group(2)).strip()[:90] if mm else ""
            trucks_by_date.setdefault(d, []).append(alt)
            if alt not in truck_info:
                truck_info[alt] = {
                    "name": alt, "dish": dish,
                    "desc": desc or "Sankei Building regular vendor",
                    "img": urljoin(url, img["src"]),
                    "link": f"{url}#{code}",
                }
    return trucks_by_date, truck_info
...
def parse_kawabata(list_url, dates):
    ...
    day_hdr_re = re.compile(
        r"(\u6708\u66dc\u65e5monday|\u706b\u66dc\u65e5tuesday|\u6c34\u66dc\u65e5wednesday|"
        r"\u6728\u66dc\u65e5thursday|\u91d1\u66dc\u65e5friday)", re.I)
    headers = [(m.start(), m.group(1)) for m in day_hdr_re.finditer(text_html)]
    code_map = {"monday": 0, "tuesday": 1, "wednesday": 2, "thursday": 3, "friday": 4}
    ...
    for i, (pos, hdr) in enumerate(headers):
        ...
        for k, v in code_map.items():
            if k in hdr.lower():
                wd = v
        end = headers[i + 1][0] if i + 1 < len(headers) else len(text_html)
        block_html = text_html[pos:end]
        block_soup = BeautifulSoup(block_html, "html.parser")
        names = []
        for a in block_soup.find_all("a", href=re.compile(r"/list/\d+/?$")):
            nm = a.get_text(" ", strip=True)
            nm = re.split(r"\s{2,}", nm)[0].strip() or nm.strip()
            # the site repeats the name twice inside the same link; dedupe
            half = len(nm) // 2
            if half > 0 and nm[:half] == nm[half:]:
                nm = nm[:half]
            href = urljoin(list_url, a["href"])
            if nm and nm not in names:
                names.append(nm)
                truck_info[nm] = {"name": nm, "link": href}
        if wd is not None:
            trucks_by_wd[wd] = names
...
def enrich_kawabata_truck(name, info):
    ...
    desc_p = None
    if h1:
        for sib in h1.find_all_next(["p", "h2"]):
            if sib.name == "h2":
                break
            if sib.get_text(strip=True):
                desc_p = sib.get_text(" ", strip=True)
                break
    info["desc"] = (desc_p or "Kawabata Food Garden regular vendor")[:100]
    prices = soup.find_all(string=re.compile("\u5186\uff08\u7a0e\u8fbc\uff09"))
    dish = None
    if prices:
        prev = prices[0].find_previous(string=True)
        ...
        for e in entries:
            for d, label in zip(dates, date_labels):
                if e["next_date"] == d:
                    ...
                    name = info["name"] or e["raw"][:24]
                    dish = dish_from_raw(e["raw"], info["name"])
                    register_truck(name, info.get("blurb") or "Otemachi-area kitchen car",
                                    dish, e["shop_url"], info.get("img"))
                    sched[label].append(name)
        note = None
        if not entries:
